...
- Kind, Context, Decision, and Consequences must be supplied explicitly in the pull-request body.
...
- A qualifying merged pull request with missing or invalid Kind evidence, or any missing required narrative section, fails visibly.
...
3. Select exactly one canonical Narrative Kind by classifying the primary nature of the decision
   being recorded — not the artefact changed or where the change is implemented:
   ...
   human review. Never infer or fall back to a Kind from titles, diffs, paths, labels, metadata,
   prose, or repository conventions.
4. Explain why the decision was required, what was chosen, material constraints or alternatives, and
   the resulting trade-offs.
5. Do not merge while the label or any required section is missing or invalid.
